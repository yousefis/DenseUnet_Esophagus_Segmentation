...
class image_class:
    def __init__(self,CTs,GTVs,Torsos,Penalize,Surface
        ...
        self.node = collections.namedtuple('node', 'img_index CT_image GTV_image Torso_image  Penalize_image Surface_image min_torso max_torso voxel_size origin direction depth width height ct_name gtv_name torso_name penalize_name surface_name')
        self.collection=[]
    ...
    def Bspline_distort(self,CT_image, GTV_image, Torso_image, Penalize_image,Surface_image,max_dis=2):
        ...
        return CT_deformed, GTV_deformed, Torso_deformed, [],Surface_image
    ...
    def read_unpadded_image(self, img_index, deform, max_dis=0): #read images which are not padded. it is a bit slow
        ...
        Surface_image1 = sitk.ReadImage(''.join(self.Surface[int(img_index)]))
        ...
        if deform == 1:
            [CT_image1, GTV_image1, Torso_image1, Penalize_image1,Surface_image1] = self.Bspline_distort(CT_image1, GTV_image1,
                                                                                          Torso_image1,
                                                                                          Penalize_image1,Surface_image1,
                                                                                          max_dis=max_dis)

        CT_image = sitk.GetArrayFromImage(CT_image1)
        GTV_image = sitk.GetArrayFromImage(GTV_image1)
        Surface_image = sitk.GetArrayFromImage(Surface_image1)
        ...
        surface_name = self.Surface[int(img_index)]
        ...
        n = self.node(img_index=img_index, CT_image=CT_image, GTV_image=GTV_image, Torso_image=Torso_image_mul,
                      Penalize_image=[],Surface_image=Surface_image,
                      ...
                      torso_name=torso_name,Surface_name=surface_name)
        return n
    # --------------------------------------------------------------------------------------------------------
    #read information of each image
    def read_image(self,img_index,deform,max_dis=0): #read padded images
        ...
        if deform==1 :
            [CT_image1,GTV_image1,Torso_image1,Penalize_image1,Surface_image1]=self.Bspline_distort(CT_image1,GTV_image1,Torso_image1,Penalize_image1,Surface_image1,max_dis=max_dis)
        ...
        height = GTV_image.shape[2]
        n = self.node(img_index=img_index, CT_image=CT_image, GTV_image=GTV_image, Torso_image=Torso_image_mul,
                      Penalize_image=Penalize_image,Surface_image=Surface_image,
                      min_torso=min_torso,max_torso=max_torso,
                      voxel_size=voxel_size, origin=origin, direction=direction,
                      depth=depth, width=width,height=height,ct_name=ct_name,
                      gtv_name=gtv_name, torso_name=torso_name,penalize_name=penalize_name,surface_name=surface_name)
        return n
    ...
    def shuffle_lists(self, CT_image_patchs, GTV_patchs,Penalize_patchs,Surface_patchs):
            ...
            Penalize_patchs1 = np.vstack([Penalize_patchs[sn]]
                                             for sn in index_shuf)
            Surface_patchs1 = np.vstack([Surface_patchs[sn]]
                                         for sn in index_shuf)
            return CT_image_patchs1, GTV_patchs1,Penalize_patchs1,Surface_patchs1
    #--------------------------------------------------------------------------------------------------------
    ...
    def read_patche_online_from_image_bunch_vl(self, sample_no_per_bunch, patch_window, GTV_patchs_size, tumor_percent,
        ...
        for ii in range(len(self.collection)):
            ...
            Surface_image = self.collection[ii].Surface_image
            ...
            Penalize_patchs1 = np.stack([(Penalize_image[
                                    ...
                                    range(len(rand_height))]).reshape(len(rand_depth), GTV_patchs_size,
                                                                      GTV_patchs_size, GTV_patchs_size)
            Surface_patchs1 = np.stack([(Surface_image[
                                          int(rand_depth[sn]) - int(GTV_patchs_size / 2) - 1:
                                          int(rand_depth[sn]) + int(GTV_patchs_size / 2),
                                          int(rand_width[sn]) - int(GTV_patchs_size / 2) - 1:
                                          int(rand_width[sn]) + int(GTV_patchs_size / 2),
            ...
            Penalize_patchs2 = np.stack([(Penalize_image[
                                    ...
                                    range(len(rand_height1))]).reshape(len(rand_depth1), GTV_patchs_size,
                                                                       GTV_patchs_size, GTV_patchs_size)
            Surface_patchs2 = np.stack([(Surface_image[
                                          int(rand_depth1[sn]) - int(GTV_patchs_size / 2) - 1:
                                          int(rand_depth1[sn]) + int(GTV_patchs_size / 2),
                                          int(rand_width1[sn]) - int(GTV_patchs_size / 2) - 1:
                                          int(rand_width1[sn]) + int(GTV_patchs_size / 2),
            ...
            if len(CT_image_patchs) == 0:
                ...
                Surface_patchs = Surface_patchs1
                Surface_patchs = np.vstack((Surface_patchs, Surface_patchs2))
            else:
                ...
                Penalize_patchs = np.vstack((Penalize_patchs, Penalize_patchs2))
                Surface_patchs = np.vstack((Surface_patchs, Surface_patchs1))
                Surface_patchs = np.vstack((Surface_patchs, Surface_patchs2))

            print(len(GTV_patchs))

        if len(GTV_patchs)!=len(Surface_patchs):
                print(1)
        CT_image_patchs1, GTV_patchs1, Penalize_patchs1,Surface_patchs1 = self.shuffle_lists(CT_image_patchs, GTV_patchs,
                                                                             Penalize_patchs,Surface_patchs)
        ...
        if self.is_training == 1:
            ...
            settings.bunch_Surface_patches2 = Surface_patchs1
            ...
            if len(settings.bunch_GTV_patches_vl2) == 0:
                ...
                settings.bunch_Surface_patches_vl2 = Surface_patchs1
            else:
                ...
                settings.bunch_Surface_patches_vl2 = np.vstack((settings.bunch_Surface_patches_vl2, Surface_patchs1))


        settings.vl_isread=True
        settings.read_patche_mutex_vl.release()
        if len(settings.bunch_CT_patches_vl2) != len(
                settings.bunch_GTV_patches_vl2) or \
                len(settings.bunch_Penalize_patches_vl2)!=len(settings.bunch_GTV_patches_vl2 )or \
                len(settings.bunch_Surface_patches_vl2)!=len(settings.bunch_GTV_patches_vl2 ):
            print('smth wrong')
    ...
    def read_patche_online_from_image_bunch(self, sample_no_per_bunch,patch_window,GTV_patchs_size,tumor_percent,img_no):
        ...
        CT_image_patchs1, GTV_patchs1,Penalize_patchs1,Surface_patchs1=self.shuffle_lists( CT_image_patchs, GTV_patchs,Penalize_patchs,Surface_patchs)
        ...
        settings.read_patche_mutex_tr.release()
        if len(settings.bunch_CT_patches_vl2)!=len(settings.bunch_GTV_patches_vl2) or len(settings.bunch_Penalize_patches_vl2)!=len(settings.bunch_GTV_patches_vl2 )or len(settings.bunch_Surface_patches_vl2)!=len(settings.bunch_GTV_patches_vl2 ):
            print('smth wrong')
    ...
    def return_patches(self,batch_no):
        ...
        if len(settings.bunch_CT_patches)>=batch_no and\
            ...
            Surface_patch=settings.bunch_Surface_patches[0:batch_no]
            ...
            settings.bunch_Surface_patches=np.delete(settings.bunch_Surface_patches,range(batch_no),axis=0)
            GTv_patch = np.eye(2)[GTv_patch]
            CT_patch = CT_patch[..., np.newaxis]
            Penalize_patch = Penalize_patch[..., np.newaxis]
            Surface_patch = Surface_patch[..., np.newaxis]
            ...
            settings.bunch_Surface_patches = np.delete(settings.bunch_Surface_patches, range(len(settings.bunch_Surface_patches)), axis=0)
        settings.train_queue.release()
        if len(CT_patch)!=len(GTv_patch) |len(CT_patch)!=len(Penalize_patch)|len(CT_patch)!=len(Surface_patch):
            print('smth wrong')


        return CT_patch,GTv_patch,Penalize_patch,loss_coef,Surface_patch
    ...
    def return_patches_validation(self, start,end):
            ...
            if (len(settings.bunch_CT_patches_vl)-(end)) >= 0\
                    ...
                    and (len(settings.bunch_Surface_patches_vl) - (end)) >= 0:
                ...
                Surface_patch = settings.bunch_Surface_patches_vl[start:end]

                if len(CT_patch) != len(GTv_patch) or len(Penalize_patch) != len(CT_patch) or len(Surface_patch) != len(CT_patch):
                    print('smth wrong')
            ...
            return CT_patch, GTv_patch,Penalize_patch,Surface_patch
    # -------------------------------------------------------------------------------------------------------
