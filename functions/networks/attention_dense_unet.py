...
import time
import functions.attention as attention
...
class _densenet_unet:
    ...
    def transition_layer(self,
                         ...
                         strides=(2, 2, 2),
                         bn_flag = False):
        with tf.variable_scope(transition_name):
    ...
    def dense_block(self, input,
        ...
        # cbam_block = attention.cbam_block(input,  name+ 'cbam_block' , ratio=8)
        # db_concat = tf.concat([tf.add(input, cbam_block), db_conv2], 4)
    ...
    def dens_net(self, image, is_training, dropout_rate1,dropout_rate2, dim,is_training_bn):
        ...
        cbam_block = attention.cbam_block(noisy_img, 'cbam_block0', ratio=1)
        noisy_mul=tf.add(noisy_img,cbam_block)
        bn1 = tf.concat([noisy_mul ,bn1], 4)
        ...
        cbam_block = attention.cbam_block(conc2, 'cbam_block_1'+self.log_ext, ratio=8)
        conc11 = tf.concat([tf.add(conc2 , cbam_block), deconv1], 4)
        ...
        cbam_block = attention.cbam_block(conc1, 'cbam_block_2'+self.log_ext, ratio=8)
        conc22 = tf.concat([tf.add(conc1 , cbam_block), deconv2], 4)
