...
if __name__=='__main__':
    ...
    if LC==1:
        ...
        fig, axes = plt.subplots(nrows=2, ncols=col, figsize=(9, 4))
        bplot1= plot_bp(axes[0,0], 'DSC -LC',dcs_without_lc)
        bplot2= plot_bp(axes[0,1], 'DSC +LC',dcs_with_lc)
        # fill with colors
        colors = ['pink', 'lightblue', 'lightgreen','orchid','tomato','hotpink','cyan']
        fill_pb_color((bplot1,bplot2), colors, axes[0,1])
        hgrid(axes[0,0],top=1,bottom=0,major_gap=.1,minor_gap=0.05,axis='y')
        hgrid(axes[0,1],top=1,bottom=0,major_gap=.1,minor_gap=0.05,axis='y')
        # =============================================
        # read xls files and fill vectors
        msd_without_lc, msd_with_lc = read_xls(xls_path, results, parent_pth, fields=['msd', 'msd_lc'])
        bplot1 = plot_bp(axes[0,2], 'MSD -LC', msd_without_lc)
        bplot2 = plot_bp(axes[0,3], 'MSD +LC', msd_with_lc)
        # fill with colors
        # colors = ['pink', 'lightblue', 'lightgreen']
        fill_pb_color((bplot1, bplot2), colors, axes[0,3])
        ...
        hgrid(axes[0,2],top=top1,bottom=0,major_gap=major_gap,minor_gap=20,axis='y')
        hgrid(axes[0,3],top=top1,bottom=0,major_gap=major_gap,minor_gap=20,axis='y')
        ...
        bplot1 = plot_bp(axes[1,0], '95%HD -LC', hd_without_lc)
        bplot2 = plot_bp(axes[1,1], '95%HD +LC', hd_with_lc)
        # fill with colors
        # colors = ['pink', 'lightblue', 'lightgreen']
        fill_pb_color((bplot1, bplot2), colors, axes[1,1])
        hgrid(axes[1,0], top=250, bottom=0, major_gap=100, minor_gap=20,axis='y')
        hgrid(axes[1,1], top=250, bottom=0, major_gap=100, minor_gap=20,axis='y')
        cumulative_dice(axes[1, 2], cnn_tags, data=dcs_without_lc, title='Cumulative dice -LC')
        cumulative_dice(axes[1, 3], cnn_tags, data=dcs_with_lc, title='Cumulative dice +LC')
        bp1 = hgrid(axes[1, 2], top=top, bottom=0, major_gap=100, minor_gap=20, axis='y')
        ...
        hgrid(axes[4], top=20, bottom=-20, major_gap=10, minor_gap=2, axis='y')
