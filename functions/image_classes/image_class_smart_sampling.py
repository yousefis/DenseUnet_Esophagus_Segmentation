...
class image_class:
    def __init__(self, CTs, GTVs, Torsos, Penalize
        ...
        self.node = collections.namedtuple('node',
                                           'img_index CT_image GTV_image Torso_image  Penalize_image min_torso max_torso voxel_size origin direction depth width height ct_name gtv_name torso_name penalize_name')
        self.collection = []
    ...
    def read_bunch_of_images_from_all_datasets(self):  # for training
        ...
        distinct_patient_1st_dataset = np.random.randint(0, 13, int(
            self.bunch_of_images_no / 3))  # max:14#select ? distinct patients from the 1st dataset
        scan_1st_dataset = np.random.randint(0, 26, int(
            self.bunch_of_images_no / 3))  # max:24#select ? numbers for determining which scan
        rand_image_no1 = distinct_patient_1st_dataset * 24 + scan_1st_dataset
        rand_image_no_2 = np.random.randint(300, 395, int(
            self.bunch_of_images_no / 3))  # select ? patient from the second dataset
        rand_image_no_3 = np.random.randint(395, len(self.CTs),
                                            int(self.bunch_of_images_no / 3))  # select ? patient from the third dataset
    ...
    def read_bunch_of_images_from2nd_dataset(self):  # for training
        ...
        rand_image_no_2 = np.random.randint(0, len(self.CTs),
                                            int(self.bunch_of_images_no))  # select 24 patient from the second dataset
    ...
    def read_patche_online_from_image_bunch_vl(self, sample_no_per_bunch, patch_window, GTV_patchs_size, tumor_percent,
        ...
        for ii in range(len(self.collection)):
            ...
            for sn in range(len(rand_height)):
                ...
                int(rand_height[sn]) - int(patch_window / 2) - 1: int(rand_height[sn]) + int(
                    patch_window / 2)]).shape != (patch_window, patch_window, patch_window):
            ...
            CT_image_patchs2 = np.stack(
                [(
                    CT_image[
                    int(rand_depth1[sn]) - int(patch_window / 2) - 1:int(rand_depth1[sn]) + int(patch_window / 2),
    ...
    # --------------------------------------------------------------------------------------------------------
    # read patches from the images which are in the RAM
    def read_patche_online_from_image_bunch(self, sample_no_per_bunch, patch_window, GTV_patchs_size, tumor_percent,
                                            other_percent, img_no):
        ...
        settings.read_patche_mutex_tr.release()
        if len(settings.bunch_CT_patches_vl2) != len(settings.bunch_GTV_patches_vl2) or len(
                settings.bunch_Penalize_patches_vl2) != len(settings.bunch_GTV_patches_vl2):
            print('smth wrong')
    ...
    def return_patches(self, batch_no):
        ...
        if len(settings.bunch_CT_patches) >= batch_no and \
            ...
            loss_coef = (np.asarray(
                [[len(np.where(GTv_patch[i, :, :, :] == 1)[0]) / np.power(self.gtv_patch_window, 3)] for i in
                 range(GTv_patch.shape[0])]))
            loss_coef = np.hstack((loss_coef, 1 - loss_coef))  # tumor vs non-tumor

        else:
            settings.bunch_CT_patches = np.delete(settings.bunch_CT_patches, range(len(settings.bunch_CT_patches)),
                                                  axis=0)
            settings.bunch_GTV_patches = np.delete(settings.bunch_GTV_patches, range(len(settings.bunch_GTV_patches)),
                                                   axis=0)
